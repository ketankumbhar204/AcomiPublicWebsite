...
assert(enquire.includes('ALREADY_DELIVERED'), 'channel duplicate must use ALREADY_DELIVERED');
assert(enquire.includes('appAlready'), 'APP already-sent step missing');
assert(enquire.includes('emailAlready'), 'EMAIL already-sent step missing');
assert(enquire.includes('deliveryChannel'), 'create must pass deliveryChannel');
assert(enquire.includes('enquireAlreadyAppTitle'), 'APP already-sent title missing');
assert(enquire.includes('enquireAlreadyEmailTitle'), 'EMAIL already-sent title missing');
assert(enquire.includes('enquireWantEmailInstead'), 'cross-channel email CTA missing on APP already-sent');
assert(enquire.includes('enquireWantAppInstead'), 'cross-channel APP CTA missing on EMAIL already-sent');
assert(en.discovery.enquireAlreadyAppTitle.toLowerCase().includes('acomi app'), 'APP already title missing');
assert(en.discovery.enquireAlreadyEmailTitle.toLowerCase().includes('email'), 'EMAIL already title missing');
...
assert(places.includes('takeEnquireResumeIntent'), 'Places page must restore listing after login');
assert(meals.includes('takeEnquireResumeIntent'), 'Meals page must restore listing after login');
...
assert(!types.includes('ownerMobile'), 'public auth types must not model ownerMobile');
assert(types.includes('ownerContact?:'), 'enquiry ownerContact is gated under ownerContact');
assert(!types.includes('Contact 3'), 'public auth types must not model Contact 3 label');
