...
function translateComplaintStatus(status: string, t: (key: string) => string): string {
  switch (status) {
    case 'Open':
    ...
    <section className="bg-[#F7F8FA] py-10 sm:py-12" aria-labelledby="complaints-heading">
      <Container>
        ...
        <div className="mt-6 grid gap-4 lg:grid-cols-2">
          <Issue badge="MESS" item={DEMO.complaints.mess} translateStatus={(s) => translateComplaintStatus(s, t)} />
          <Issue badge="PG" item={DEMO.complaints.lodging} translateStatus={(s) => translateComplaintStatus(s, t)} />
        </div>
      </Container>
    </section>
  );
