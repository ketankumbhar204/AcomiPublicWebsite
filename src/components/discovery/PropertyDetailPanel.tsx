...
import { amenityLabel, formatInr } from '../../data/listings/query';
...
export function PropertyDetailPanel({ listing, onEnquire }: PropertyDetailPanelProps) {
  ...
  const facts = [
    { label: propertyAvailabilityLabel(listing), Icon: Users },
    listing.amenityCodes.includes('FOOD_INCLUDED')
      ? { label: 'Meals available', Icon: UtensilsCrossed }
      : null,
    listing.amenityCodes.includes('WIFI') ? { label: 'Wi-Fi', Icon: Wifi } : null,
    listing.amenityCodes.includes('POWER_BACKUP') ? { label: 'Power backup', Icon: Zap } : null,
  ...
  return (
    <div className="flex h-full flex-col">
      ...
      <div className="mt-5">
        <h3 className="text-sm font-semibold text-navy">About this place</h3>
